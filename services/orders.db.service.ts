...
export async function getAllOrders(limit = 100, endingPoint: string | undefined = undefined): Promise<Order[]> {
    ...
    if (endingPoint == undefined) {
        resp = await databasePool.query<Order>(`SELECT * FROM "Orders" WHERE "orderId" < $2 ORDER BY "orderId" DESC LIMIT $1`, [limit, endingPoint]);
    } else {
        resp = await databasePool.query<Order>(`SELECT * FROM "Orders" WHERE "orderId" < $2 ORDER BY "orderId" DESC LIMIT $1`, [limit, endingPoint]);
    }
    return resp.rows;
...
export async function getOrdersFromBuyerAddress(buyerAddress: string): Promise<Order[]> {
    ...
    let resp = await databasePool.query<Order>(`SELECT * FROM "Orders" WHERE "buyerAddress" = $1`, [buyerAddress]);
    return resp.rows;
}

export async function getOrdersFromListingId(listingId: bigint): Promise<Order[]> {
    if (listingId <= 0) return [];
    let resp = await databasePool.query<Order>(`SELECT * FROM "Orders" WHERE "listingId" = $1`, [listingId]);
    return resp.rows;
}
...
export async function getOrdersFromSellerAddress(sellerAddress: string): Promise<Order[]> {
    ...
    let resp = await databasePool.query<Order>(`SELECT "Orders".* FROM "Orders", "Listings" WHERE "Orders"."listingId" = "Listings"."ListingId" AND "sellerAddress" = $1`, [sellerAddress]);
    return resp.rows;
}

export async function getOrderDetailsFromId(orderId: bigint): Promise<false | Order> {
    if (orderId <= 0) return false;
    let resp = await databasePool.query<Order>(`SELECT * FROM "Orders" WHERE "orderId" = $1`, [orderId]);
...
export async function setOrderDetails(orderId: bigint, listingId: bigint, buyerAddress: string, buyerInfo: string) : Promise<boolean> {
    ...
    await databasePool.query(`INSERT INTO "Orders"("orderId", "listingId", "buyerAddress", "buyerInfo") VALUES($1, $2, $3, $4);`, [orderId, listingId, buyerAddress, buyerInfo]);
    return true;
}
...
export async function createTable() {
    ...
    await databasePool.query(`ALTER TABLE "Orders" ADD CONSTRAINT "listingId" FOREIGN KEY ("listingId") REFERENCES "Listings" ("ListingId") ON DELETE CASCADE;`);
}